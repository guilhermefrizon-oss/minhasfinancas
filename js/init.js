...
document.addEventListener('DOMContentLoaded', () => {
  ...
  ['in-desc','in-cat','in-pag','in-valor','in-dia-venc','in-mes-ini','in-parcela-mes','in-parcelas-total'].forEach(id=>{
    const field=document.getElementById(id);
    if(field){field.addEventListener('input',()=>requestAnimationFrame(()=>{updateNewRecurringSummary();updateNewInstallmentSummary();}));field.addEventListener('change',()=>{updateNewRecurringSummary();updateNewInstallmentSummary();});}
  });
});
